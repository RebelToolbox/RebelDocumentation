...
.. seealso::
    ...
    creating and editing themes, see :doc:`/tutorials/ui/gui_skinning`.
...
placeholder ``%s``. When using this feature, Rebel Engine automatically converts values
to text, which is convenient to output text in labels or using the ``print()``
function.

.. seealso::

    You can learn more about string formatting here: :doc:`/tutorials/scripting/gdscript/gdscript_format_string`.
...
another feature in Rebel Engine: :doc:`autoloads </tutorials/scripting/singletons_autoload>`.
...
You can use the autoload feature to have Rebel Engine load a node or a scene
...
We included a music soundtrack in the ``audio/`` directory, ``House In a Forest
