...
Rebel Engine uses `SCons <https://www.scons.org/>`__ to build. We love it, we are
not changing it for anything else. We are not even sure other build
systems are up to the task of building Rebel Engine. We constantly get requests
...
-  Rebel Engine can be compiled for a dozen different platforms: all PC
...
-  Rebel Engine build process is not simple. Several files are generated by
...
-  Rebel Engine build process makes heavy use of cross-compiling tools. Each
...
if you are planning to build Rebel Engine yourself.
...
Rebel Engine's build system will begin by detecting the platforms it can build
...
generally with this naming convention::
    ...
    rebel.<platform>.[opt].[tools/debug].<architecture>[extension]

For the previous build attempt, the result would look like this::

    ls bin
    bin/rebel.x11.tools.64
...
.. code-block:: console
    ...
    C:\RebelEngine> dir bin/
    rebel.windows.tools.64.exe
...
can be either downloaded from `rebeltoolbox.com <https://rebeltoolbox.com/>`__, or
...
It's possible to compile modules residing outside of Rebel Engine's directory
...
::
    ...
    scons custom_modules="../modules,/abs/path/to/modules,~/src/rebel_modules"
...
list of build options you've used to build Rebel Engine previously.
...
way Rebel Engine should be built (compiler, debug options, etc.) as well as the
...
The default ``custom.py`` file can be created at the root of the Rebel Engine
...
For instance, you may want to build Rebel Engine in parallel with the aforementioned
...
Official export templates are downloaded from the Rebel Toolbox site:
`rebeltoolbox.com <https://rebeltoolbox.com/>`__. However, you might want
...
The ``version.txt`` file should contain the corresponding Rebel Engine version
