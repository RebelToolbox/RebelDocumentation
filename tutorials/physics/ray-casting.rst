...
Rebel Editor stores all the low level game information in servers, while the
...
In the physics world, Rebel Editor stores all the low level collision and
...
Rebel Physics runs by default in the same thread as game logic, but may
...
See :ref:`tutorials/physics/physics_introduction:code example` for details on how to set the collision mask.
