...
With :ref:`AnimationPlayer <class_AnimationPlayer>`, Rebel Engine has one of the most flexible animation systems that you can find in any game engine.
...
:ref:`AnimationTree <class_AnimationTree>` deals with advanced transitions.
...
At the end, the imported Rebel scene will contain the animations in a ``AnimationPlayer`` node.

As you rarely use imported scenes directly in Rebel Engine (they are either instantiated or inherited from), you can place the ``AnimationTree`` node in your
...
Here a new scene was created for the player with a ``KinematicBody`` as root. Inside this scene, the original ``.dae`` (Collada) file was instantiated
...
When playing back the animation in Rebel Engine, it is possible to select this bone as the *root motion track*. Doing so will cancel the bone
...
One of the nice features in Rebel Engine in the ``StateMachine`` implementation is the ability to travel. The graph can be instructed to go from the
